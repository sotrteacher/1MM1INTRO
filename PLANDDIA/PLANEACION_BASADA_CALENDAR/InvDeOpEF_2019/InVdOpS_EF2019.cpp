// Proyecto_Calendario_Planeacion1.cpp: define el punto de entrada de la aplicaci\'on de consola.
//
...
#ifdef VISUAL_STUDIO_2017
TCHAR STR[] = _T("pdflatex.exe PlaneacionInvDeOpCOMP_2.tex");
TCHAR STR1[] = _T("AcroRd32.exe PlaneacionInvDeOpCOMP_2.pdf");
#else
...
#define NUM_INIC_DE_DIA_PLANEADO      24
/*
using std::cout;
using std::cerr;
using std::endl;
...
int main(int argc, char** argv) {
...
#ifdef INVESTIGACION_DE_OPERACIONES_COMP
	Fecha f1(1, 0); /*Martes 1 de enero de 2019 */
	Fecha f2(4, 1); /*Lunes 4 de febrero de 2019 */
	...
	vector<Dia*> vdnl;
	vdnl.push_back(new Dia(new Fecha(1,0))); /*Martes 1 de enero de 2019*/
...
/*Ahora usando el vector de Fechas VDF y el vector de Dias no 
 ...
 Calendario*/
	...
	//NACT("III.C RESOLUCI\\'ON DE PROBLEMAS LI\\-NEA\\-LES",4.0);
	...
	NACT("V.H M\\'ETODO HUNGARO",2.0);//0+2=2
	NACT("V.I RESOLUCI\\'ON DE PROBLEMAS POR LA RUTA M\\'AS CORTA",2.0);//2+2=4
	NACT("V.J RESOLUCI\\'ON DE PROBLEMAS POR LA RUTA M\\'AS LARGA",2.0);//4+2=6
	NACT("V.K FORMULADO DE PROBLEMAS POR FLUJO M\\'AXIMO",2.0);//6+2=8
	NACT("V.L PLANTEAMIENTO DE PROBLEMAS",2.0);//8+2.0=10.0
	NACT("TERCERA EVALUACI\\'ON PARCIAL",2.0);//10.0+2=12.0
	NACT("REVISI\\'ON DE LA TERCERA EVALUACI\\'ON PARCIAL", 1.0);//12.0+1=13.0
	...
	actividaes correspondientes por dia de clase entre las fechas 
	f1 y f2.*/
